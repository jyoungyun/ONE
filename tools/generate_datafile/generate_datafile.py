...
if __name__ == "__main__":
  parser = argparse.ArgumentParser(description='Generate Model input and expected datas for onert_train')
  ...
  for idx in range(args.num_runs):
    ...
    subprocess.run([args.tool, '--modelfile', args.model,
                    '--dump_input:raw', input_idx_file,
                    '--dump:raw', output_idx_file])
  
    input_idx_file += '.0'
    output_idx_file += '.0'

    if os.path.exists(input_idx_file) == False or os.path.exists(output_idx_file) == False:
